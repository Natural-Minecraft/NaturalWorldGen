...
import id.naturalsmp.nwg.util.reflect.V;
...
public abstract class NaturalDevPlugin extends JavaPlugin implements Listener {
    ...
    private void registerCommands() {
        ...
        for (Field i : getClass().getDeclaredFields()) {
            if (i.isAnnotationPresent(id.naturalsmp.nwg.util.plugin.Command.class)) {
                try {
                    i.setAccessible(true);
                    MortarCommand pc = (MortarCommand) i.getType().getConstructor().newInstance();
                    id.naturalsmp.nwg.util.plugin.Command c = i.getAnnotation(id.naturalsmp.nwg.util.plugin.Command.class);
