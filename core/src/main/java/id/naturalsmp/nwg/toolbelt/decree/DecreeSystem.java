...
package id.naturalsmp.nwg.toolbelt.decree;
...
public interface DecreeSystem extends CommandExecutor, TabCompleter {
    KList<DecreeParameterHandler<?>> handlers = NaturalGenerator.initialize("id.naturalsmp.nwg.toolbelt.decree.handlers", null).convert((i) -> (DecreeParameterHandler<?>) i);
    ...
    default boolean call(NaturalDevSender sender, String[] args) {
    ...
    default List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        DecreeContext.touch(new NaturalDevSender(sender));
        try {
    ...
    default boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (!sender.hasPermission("naturalworldgen.all")) {
            sender.sendMessage("You lack the Permission 'naturalworldgen.all'");
            return true;
        }

        J.aBukkit(() -> {
            var naturalsmp = new NaturalDevSender(sender);
            if (!call(naturalsmp, args)) {
                ...
                naturalsmp.sendMessage(C.RED + "Unknown NaturalGenerator Command");
            } else {
                if (IrisSettings.get().getGeneral().isCommandSounds()) {
                    if (sender instanceof Player) {
