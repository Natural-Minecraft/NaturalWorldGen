package id.naturalsmp.nwg.utilities.plugin.chunk;
...
public class TicketHolder {
    ...
    public void addTicket(int x, int z) {
        tickets.compute(Cache.key(x, z), ($, ref) -> {
            if (ref == null) {
                world.addPluginChunkTicket(x, z, NaturalGenerator.instance);
                return 1L;
            }
            return ++ref;
        });
    ...
    public boolean removeTicket(int x, int z) {
        return tickets.compute(Cache.key(x, z), ($, ref) -> {
            ...
            if (--ref <= 0) {
                world.removePluginChunkTicket(x, z, NaturalGenerator.instance);
                return null;
            }
            return ref;
        }) == null;
