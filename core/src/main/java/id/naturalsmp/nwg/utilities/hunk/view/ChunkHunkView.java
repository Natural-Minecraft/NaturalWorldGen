...
package id.naturalsmp.nwg.utilities.hunk.view;

import id.naturalsmp.nwg.NaturalGenerator;
import id.naturalsmp.nwg.core.service.EditSVC;
...
public class ChunkHunkView implements Hunk<BlockData> {
    ...
    public void setRaw(int x, int y, int z, BlockData t) {
        ...
        NaturalGenerator.service(EditSVC.class).set(chunk.getWorld(), x + (chunk.getX() * 16), y, z + (chunk.getZ() * 16), t);
    }

    @Override
    public BlockData getRaw(int x, int y, int z) {
        return NaturalGenerator.service(EditSVC.class).get(chunk.getWorld(), x + (chunk.getX() * 16), y, z + (chunk.getZ() * 16));
    }
}
