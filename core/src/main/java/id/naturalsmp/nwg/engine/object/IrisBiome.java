...
import id.naturalsmp.nwg.core.gui.components.RenderType;
...
import id.naturalsmp.nwg.util.data.VanillaBiomeMap;
import id.naturalsmp.nwg.util.inventorygui.RandomColor;
...
@NoArgsConstructor
@AllArgsConstructor
@Desc("Represents a biome in naturalworldgen. Biomes are placed inside of regions and hold objects.\nA biome consists of layers (block palletes), decorations, objects & generators.")
@Data
...
public class IrisBiome extends IrisRegistrant implements IRare {
    ...
    @Desc("The default slab if naturalworldgen decides to place a slab in this biome. Default is no slab.")
    private IrisBiomePaletteLayer slab = new IrisBiomePaletteLayer().zero();
    @Desc("The default wall if naturalworldgen decides to place a wall higher than 2 blocks (steep hills or possibly cliffs)")
    ...
    @Desc("Objects define what schematics (iob files) naturalworldgen will place in this biome")
                ...
                return this.cacheColor.aquire(() -> {
                    if (this.color == null) {
                        ...
                        if (this.getVanillaDerivative() == null) {
                            NaturalGenerator.warn("No vanilla biome found for " + getName());
                    ...
                    try {
                        ...
                        NaturalGenerator.warn("Could not parse color \"" + this.color + "\" for biome " + getName());
