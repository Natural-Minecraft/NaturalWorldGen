...
import id.naturalsmp.nwg.core.nms.container.BlockPos;
...
import id.naturalsmp.nwg.engine.framework.*;
import id.naturalsmp.nwg.engine.mantle.EngineMantle;
import id.naturalsmp.nwg.engine.object.*;
import id.naturalsmp.nwg.util.atomics.AtomicRollingSequence;
...
import id.naturalsmp.nwg.util.matter.MatterStructurePOI;
import id.naturalsmp.nwg.util.matter.slices.container.JigsawStructureContainer;
...
public class IrisEngine implements Engine {
    ...
    public IrisEngineData getEngineData() {
        return engineData.aquire(() -> {
            ...
            File f = new File(getWorld().worldFolder(), "naturalworldgen/engine-data/" + getDimension().getLoadKey() + ".json");
            ...
            if (data == null) {
                ...
                data.getStatistics().setVersion(NaturalGenerator.instance.getIrisVersion());
    ...
    private boolean EngineSafe() {
        ...
        int MinecraftVersion = NaturalGenerator.instance.getMCVersion();
        int IrisVersion = NaturalGenerator.instance.getIrisVersion();
