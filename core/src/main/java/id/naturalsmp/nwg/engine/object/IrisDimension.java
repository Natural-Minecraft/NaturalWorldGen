...
import id.naturalsmp.nwg.core.ServerConfigurator.DimensionHeight;
...
import id.naturalsmp.nwg.core.nms.datapack.IDataFixer.Dimension;
import id.naturalsmp.nwg.engine.data.cache.AtomicCache;
import id.naturalsmp.nwg.engine.object.annotations.*;
import id.naturalsmp.nwg.engine.object.annotations.functions.ComponentFlagFunction;
...
public class IrisDimension extends IrisRegistrant {
    ...
    @Desc("Keep this either undefined or empty. Setting any biome name into this will force naturalworldgen to only generate the specified biome. Great for testing.")
    private String focus = "";
    @RegistryListResource(IrisRegion.class)
    @Desc("Keep this either undefined or empty. Setting any region name into this will force naturalworldgen to only generate the specified region. Great for testing.")
    ...
    @MinNumber(0)
    @MaxNumber(8192)
    @Desc("Coordinate fracturing applies noise to the input coordinates. This creates the 'naturalworldgen swirls' and wavy features. The distance pushes these waves further into places they shouldnt be. This is a block value multiplier.")
    ...
    public static void writeShared(KList<File> folders, DimensionHeight height) {
        ...
        for (File datapacks : folders) {
            File mcm = new File(datapacks, "naturalworldgen/pack.mcmeta");
            try {
                IO.writeAll(mcm, raw);
            } catch (IOException e) {
                NaturalGenerator.reportError(e);
    ...
    private static void write(File datapacks, String type, String json) {
        if (json == null) return;
        File dimTypeVanilla = new File(datapacks, "naturalworldgen/data/minecraft/dimension_type/" + type + ".json");
