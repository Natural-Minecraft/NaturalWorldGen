...
public class IrisWorlds {
    ...
    public static IrisWorlds get() {
        return cache.aquire(() -> {
            File file = NaturalGenerator.instance.getDataFile("worlds.json");
            if (!file.exists()) {
    ...
    public void clean() {
        dirty = worlds.entrySet().removeIf(entry -> !new File(Bukkit.getWorldContainer(), entry.getKey() + "/naturalworldgen/pack/dimensions/" + entry.getValue() + ".json").exists());
    }
