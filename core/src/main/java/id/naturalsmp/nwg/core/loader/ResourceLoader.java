...
import id.naturalsmp.nwg.util.io.CustomOutputStream;
...
public class ResourceLoader<T extends IrisRegistrant> implements MeteredCache {
    ...
    public JSONObject buildSchema() {
        ...
        o.put("url", "./.naturalworldgen/schema/" + getFolderName() + "-schema.json");
        File a = new File(getManager().getDataFolder(), ".naturalworldgen/schema/" + getFolderName() + "-schema.json");
    ...
    public void loadFirstAccess(Engine engine) throws IOException {
        ...
        File file = NaturalGenerator.instance.getDataFile("prefetch/" + id + "/" + Math.abs(getFolderName().hashCode()) + ".ipfch");
