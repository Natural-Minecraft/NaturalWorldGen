...
import id.naturalsmp.NaturalWorldGen.util.reflect.V;
...
public abstract class NaturalDevPlugin extends JavaPlugin implements Listener {
    ...
    private void registerCommands() {
        ...
        for (Field i : getClass().getDeclaredFields()) {
            if (i.isAnnotationPresent(id.naturalsmp.NaturalWorldGen.util.plugin.Command.class)) {
                try {
                    i.setAccessible(true);
                    MortarCommand pc = (MortarCommand) i.getType().getConstructor().newInstance();
                    id.naturalsmp.NaturalWorldGen.util.plugin.Command c = i.getAnnotation(id.naturalsmp.NaturalWorldGen.util.plugin.Command.class);
