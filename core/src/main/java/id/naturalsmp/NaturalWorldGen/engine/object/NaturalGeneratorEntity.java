...
import id.naturalsmp.NaturalWorldGen.util.plugin.Chunks;
...
import static id.naturalsmp.NaturalWorldGen.util.data.registry.Particles.ITEM;
...
@Desc("Represents an naturalworldgen entity.")
@Data
...
public class IrisEntity extends IrisRegistrant {
            ...
            if (getLoot().getTables().isNotEmpty()) {
                ...
                l.setLootTable(new LootTable() {
                    ...
                    public NamespacedKey getKey() {
                        return new NamespacedKey(NaturalGenerator.instance, "loot-" + IrisEntity.this.hashCode());
                    }
    ...
    private Entity doSpawn(Location at) {
        ...
        if (!Bukkit.isPrimaryThread()) {
            ...
            // Due to the structure of naturalworldgen, we will call it sync and busy wait until it's done.
