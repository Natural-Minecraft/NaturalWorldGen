...
import id.naturalsmp.NaturalWorldGen.util.scheduling.O;
import id.naturalsmp.NaturalWorldGen.util.scheduling.PrecisionStopwatch;
import id.naturalsmp.NaturalWorldGen.util.scheduling.jobs.Job;
import id.naturalsmp.NaturalWorldGen.util.scheduling.jobs.JobCollection;
import id.naturalsmp.NaturalWorldGen.util.scheduling.jobs.ParallelQueueJob;
import lombok.Data;
...
public class IrisProject {
    ...
    public static int clean(NaturalDevSender s, File clean) {
        int c = 0;
    ...
    public void open(NaturalDevSender sender) throws IrisException {
        open(sender, 1337, (w) ->
        {
        });
    }

    public void openVSCode(NaturalDevSender sender) {
    ...
    public void open(NaturalDevSender sender, long seed, Consumer<World> onDone) throws IrisException {
        if (isOpen()) {
            close();
        }
        ...
        J.a(() -> {
            ...
            try {
                activeProvider = (PlatformChunkGenerator) IrisToolbelt.createWorld()
                        ...
                        .sender(sender)
                        .studio(true)
                        .name("naturalworldgen/" + UUID.randomUUID())
    ...
    public void close() {
        ...
        NaturalGenerator.linkMultiverseCore.removeFromConfig(activeProvider.getTarget().getWorld().name());
    ...
    public JSONObject createCodeWorkspaceConfig() {
        ...
        for (Class<?> i : dm.resolveSnippets()) {
            try {
                ...
                o.put("url", "./.naturalworldgen/schema/snippet/" + snipType + "-schema.json");
                schemas.put(o);
                File a = new File(dm.getDataFolder(), ".naturalworldgen/schema/snippet/" + snipType + "-schema.json");
                J.attemptAsync(() -> {
                    try {
    ...
    public File compilePackage(NaturalDevSender sender, boolean obfuscate, boolean minify) {
        String dimm = getName();
    ...
    public void compile(NaturalDevSender sender) {
    ...
    private void scanForErrors(IrisData data, File f, JSONObject p, NaturalDevSender sender) {
    ...
    public void compare(Class<?> c, JSONObject j, NaturalDevSender sender, KList<String> path) {
        try {
