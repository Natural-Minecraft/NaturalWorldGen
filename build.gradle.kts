 ...
 * Copyright (c) 2021 Arcane Arts (NaturalDev Software)
 *
...
registerCustomOutputTask("PixelFury", "C://Users/repix/workplace/NaturalWorldGen/1.21.3 - Development-Public-v3/plugins")
registerCustomOutputTask("PixelFuryDev", "C://Users/repix/workplace/NaturalWorldGen/1.21 - Development-v3/plugins")
// ========================== UNIX ==============================
...
nmsBindings.forEach { (key, value) ->
    ...
    tasks.register<RunServer>("runServer-$key") {
        ...
        systemProperty("naturalworldgen.suppressReporting", !errorReporting)
...
tasks {
    jar {
        ...
        archiveFileName.set("NaturalWorldGen-${project.version}.jar")
    }

    register<Copy>("nwg") {
        group = "nwg"
        dependsOn("jar")
        from(layout.buildDirectory.file("libs/NaturalWorldGen-${project.version}.jar"))
    ...
    register<Copy>("irisDev") {
        ...
        rename { it.replace("core", "NaturalWorldGen-${project.version}") }
        into(layout.buildDirectory)
    ...
    register("release") {
        ...
        doLast {
            val url = "http://sentry.naturalsmp.id:8080"
...
fun registerCustomOutputTask(name: String, path: String) {
    ...
    tasks.register<Copy>("build$name") {
        ...
        dependsOn("nwg")
        from(layout.buildDirectory.file("NaturalWorldGen-${project.version}.jar"))
        into(file(path))
        rename { "NaturalWorldGen.jar" }
    }
}
